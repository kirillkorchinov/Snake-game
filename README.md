...
и использует char ```get_play_field_value(int value)``` для определения символа для вывода:
...
```find_max()``` возвращает наибольший элемент элемент массива (голова змейки) и выводит стрелку в соответствии с направлением движения змейки.
